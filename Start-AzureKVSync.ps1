...
$Primary = Get-Content -Path $ArtifactStagingDirectory\Global-$PrimaryPrefix.json | ConvertFrom-Json
$Secondary = Get-Content -Path $ArtifactStagingDirectory\Global-$SecondaryPrefix.json | ConvertFrom-Json
